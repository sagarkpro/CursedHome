...
function CardActions({ onEdit, onDelete }: { onEdit?: () => void; onDelete?: () => void }) {
	...
	return (
		<div className="absolute top-3 right-3 z-10 flex items-center gap-1.5 opacity-0 -translate-y-1 scale-95 group-hover:opacity-100 group-hover:translate-y-0 group-hover:scale-100 transition-all duration-300 ease-out">
			{onEdit && (
				<button onClick={stop(onEdit)} className="p-2 bg-black/60 hover:bg-blue-500/80 text-zinc-300 hover:text-white rounded-full backdrop-blur-md transition-colors cursor-pointer" title="Edit">
					<Edit2 className="w-4 h-4" />
				</button>
			)}
			{onDelete && (
				<button onClick={stop(onDelete)} className="p-2 bg-black/60 hover:bg-red-500/80 text-zinc-300 hover:text-white rounded-full backdrop-blur-md transition-colors cursor-pointer" title="Delete">
